...
int addEmployees(Employee* array, int len, int* id, int index)
{
	...
	{
		if(utn_getDescripcion(buffer.name, NAME_SIZE ,"Ingrese nombre", "Error, tiene un reintento", 1) == 0 &&
		   utn_getDescripcion(buffer.lastName, LASTNAME_SIZE,"Ingrese apellido", "Error, tiene un reintento", 1) == 0 &&
		   utn_getNumeroFlotante(&buffer.salary, "Ingrese salario, entre $1 y $1000000", "Error, tiene un reintento", MIN_SALARY, MAX_SALARY,1) == 0 &&
		   utn_getNumero(&buffer.sector, "Ingrese numero de sector, entre 1 y 20", "Error, reintente", MIN_SECTOR, MAX_SECTOR, 1) == 0)
		{
			retorno=0;
			buffer.isEmpty=0;
			buffer.id=(*id);
